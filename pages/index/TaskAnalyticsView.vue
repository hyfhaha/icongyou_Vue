		...
		<scroll-view scroll-y class="page-scroll">
			...
			<view class="task-header-card">
				<view class="task-header-top">
					<view>
						<text class="task-name">{{ taskDetail.name }}</text>
						<text class="task-meta">截止 {{ taskDetail.deadline }} · 核心要求 {{ taskDetail.keyRequirement }}</text>
					</view>
					<view class="status-chip" :class="taskDetail.status">
						<text>{{ getStatusText(taskDetail.status) }}</text>
					</view>
				</view>
				...
				<view class="task-info-grid">
					...
					<view class="info-item">
						...
						<text class="info-value">{{ taskDetail.points }}</text>
					</view>
					<view class="info-item">
						<text class="info-label">提交限制</text>
						<text class="info-value">{{ taskDetail.submissionLimit }}</text>
					</view>
					<view class="info-item">
						<text class="info-label">讨论活跃</text>
						<text class="info-value">{{ taskDetail.hotIndex }}</text>
					</view>
				</view>
			</view>
			...
			<view class="metric-grid">
				<view class="metric-card">
					...
					<text class="metric-value">{{ taskMetrics.submissionRate }}%</text>
					<view class="mini-progress">
						<view class="mini-progress-fill" :style="{ width: taskMetrics.submissionRate + '%' }"></view>
					</view>
					<text class="metric-footnote">{{ taskMetrics.submitted }}/{{ taskMetrics.totalStudents }} 人已提交</text>
				</view>
				<view class="metric-card">
					<text class="metric-label">未提交</text>
					<text class="metric-value" style="color:#F97316;">{{ taskMetrics.pending }} 人</text>
					<text class="metric-footnote">需督促跟进</text>
				</view>
				<view class="metric-card">
					<text class="metric-label">查看频次</text>
					<text class="metric-value">{{ taskMetrics.views }}</text>
					<text class="metric-footnote">近 7 天访问</text>
				</view>
				<view class="metric-card">
					<text class="metric-label">讨论次数</text>
					<text class="metric-value">{{ taskMetrics.discussions }}</text>
			...
			<view class="action-card">
				...
				<view class="action-item" @click="goExcellent">
					...
					<view class="action-info">
						...
						<text class="action-desc">教师推荐 6 篇作品可参考</text>
					</view>
					<uni-icons type="right" size="18" color="#AAAAAA"></uni-icons>
				</view>
...
const taskDetail = ref({
  id: 'T4-1',
  name: 'T4-1 爱从游（学生移动端）',
  deadline: '2025-11-30 00:00',
  keyRequirement: '完成多端联动与任务看板',
  status: 'in-progress',
  type: '团队任务',
  points: 200,
  submissionLimit: '不限次数',
  hotIndex: '92/100'
});
...
const goTaskDetail = () => {
  uni.navigateTo({
    url: '/pages/index/TaskDetailView?taskId=T4-1'
  });
};
